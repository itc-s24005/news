...
export default async function Page({text}: Props) {
    ...
    return (
        ...
        <div style={{ display: "flex", flexWrap: "wrap"}}>
          {newsList.map((news) => (
            <div key={news.link} style={{ margin: "15px 7px", width: "330px", height: "350px", border: "1px solid #808080", borderRadius: "30px" }}>
              <a href={news.link} style={{ padding: "0px" }}>
                ...
                <div style={{ margin: "15px 18px 0"}}>
                  ...
                  <h2 style={news.image_url ? { marginTop: "5px", fontSize: "18px", fontWeight: "bold" } : { marginTop: "5px", fontSize: "18px", fontWeight: "bold", display: "none" }}>{news.title.slice(0, 48) !== news.title ? news.title.slice(0, 48) + "..." : news.title}</h2>
                </div>
              </a>
            </div>
          ))}
